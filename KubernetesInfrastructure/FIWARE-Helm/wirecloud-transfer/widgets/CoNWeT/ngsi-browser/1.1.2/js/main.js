...
(function (mp, se) {
    ...
    NGSIBrowser.prototype.init = function init() {
        ...
        this.reload_button = new se.Button({
        ...
        this.reload_button.addEventListener('click', function () {
            reload.call();
        }.bind(this));

        this.layout.center.appendChild(this.add_entity_button);
        this.layout.center.appendChild(this.reload_button);
    };
    ...
    var onNGSIQuerySuccess = function onNGSIQuerySuccess(next, page, data, details) {
        ...
        mp.wiring.pushEvent('entities', JSON.stringify(data));
    };
    ...
    var createTable = function createTable() {
        ...
        if (mp.prefs.get('type_column')) {
            fields.push({field: 'type', label: 'Typ', sortable: false});
        }
        ...
        if (mp.prefs.get('allow_delete') || mp.prefs.get('allow_use') || mp.prefs.get('allow_diagram')) {
            fields.push({
                label: 'Aktionen',
                width: '160px',
        ...
        this.table = new se.ModelTable(fields, {
            id: 'id',
            pageSize: 20,
            source: this.ngsi_source,
            'class': 'table-striped'
        });
