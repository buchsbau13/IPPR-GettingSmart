            ...
            onSuccess: function (response) {
                ...
                */
                ...
                responseData.forEach(function (sensor) {
                    var timestamps;
                    var dataseries;
                    var dataseriesLocation;

                    responseData[sensorNum].contextElement.attributes.map(function (entry){
                       if (entry.name == "location"){
                           dataseriesLocation = entry.values.map(function (entry) {
                               return entry.attrValue;
                           });
                           console.log("location " + dataseriesLocation);
                       }
                       else {
                           timestamps = entry.values.map(function (entry) {
                               return (new Date(entry.recvTime)).getTime();
                           });
                           console.log("timestamps " + timestamps);

                           dataseries = entry.values.map(function (entry) {
                               return entry.attrValue;
                           });
                    ...
                    if (dataseries.length !== 0 && timestamps.length !== 0) {
                        console.log("forward");
                        var data = {};
                        data.entityId = sensor.contextElement.id;
