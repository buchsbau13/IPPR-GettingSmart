...
(function (mp) {
    ...
    var requestData = function requestData(entityString, attribute) {
        ...
        */
        ...
        var urlstatic = new URL('v1/contextEntities/type/' + entity_type + '/id/Sensor1,Sensor2,Sensor3,Sensor4,Sensor5,Sensor6,Sensor7,Sensor_DonBosco/attributes/' + attribute + ',humidity', server);
        var urlmobile = new URL('v1/contextEntities/type/' + entity_type + '/id/Tram1_654,Tram2_655,Bus1_30,Bus2_31/attributes/' + attribute + ',humidity', server);
        var urltraffic = new URL('v1/contextEntities/type/' + entity_type + '/id/Traffic1_west,Traffic1_east,Traffic2_west,Traffic2_east/attributes/' + attribute + ',humidity', server);
        var url;
        ...
        mp.http.makeRequest(url, {
            ...
            onSuccess: function (response) {
                ...
                */
                ...
                console.log(JSON.parse(response.responseText).contextResponses);

                var sensorNum = 0;
                responseData.forEach(function (sensor) {

                    var timestamps = responseData[sensorNum].contextElement.attributes[0].values.map(function (entry) {
                        return (new Date(entry.recvTime)).getTime();
                    });
                    console.log("timestamps " + timestamps);
                    var dataseries = responseData[sensorNum].contextElement.attributes[0].values.map(function (entry) {
                        return entry.attrValue;
                    });
                    console.log("dataseries " + dataseries);
                    var dataseriesLocation = responseData[sensorNum].contextElement.attributes[1].values.map(function (entry) {
                        return entry.attrValue;
                    });
