        ...
        mp.http.makeRequest(url, {
            ...
            onSuccess: function (response) {
                ...
                */
                ...
                console.log(JSON.parse(response.responseText).contextResponses[0].contextElement.attributes);

                // responseData.forEach(function (sensor) {

                    var timestamps = responseData[0].contextElement.attributes[0].values.map(function (entry) {
                        return (new Date(entry.recvTime)).getTime();
                    });
                    console.log("timestamps " + timestamps);
                    var dataseries = responseData[0].contextElement.attributes[0].values.map(function (entry) {
                        return entry.attrValue;
                    });
                    console.log("dataseries " + dataseries);
                    var dataseriesLocation = responseData[1].contextElement.attributes[0].values.map(function (entry) {
                        return entry.attrValue;
                    });
