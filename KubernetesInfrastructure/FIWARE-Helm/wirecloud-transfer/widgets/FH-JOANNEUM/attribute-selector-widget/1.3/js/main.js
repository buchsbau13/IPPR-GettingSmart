...
(function () {
    ...
    var init = function init() {
        ...
        var fields = {};
        var json = JSON.parse(MashupPlatform.prefs.get('attributes').trim());
        if (json) {
                ...
                device.attributes.forEach(function (attribute) {
                    if (friendlyEnt && friendlyEnt[attribute.name]) {
                        entries.push({
                            ...
                            label: friendlyEnt[attribute.name]
                        });
                    } else {
                        entries.push({
                            value: attribute.name
        ...
        form = new StyledElements.Form(fields, {
            cancelButton: false,
            acceptButton: false
        });
        json.devices.forEach(function (device) {
            form.fieldInterfaces[device.entity_type].inputElement.addEventListener('change', function () {
                onInputChange(device.entity_type);
            });
        });
